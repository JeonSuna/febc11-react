...
function App() {
  ...
  const fetchData = async (_id) => {
    ...
    try {
      ...
      console.error(err);
      setError({ message: '잠시후 다시 요청해주세요' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const basicShipppingFees = 3000;

  const [quantity, setQuantity] = useState(1);
  const [shippingFees, setShippingFees] = useState(basicShipppingFees);

  // 수량이 변경되면 배송비 다시 계산
  const handleQuantityChange = (e) => {
    const newQuantity = Number(e.target.value);
    setShippingFees(basicShipppingFees * Math.ceil(newQuantity / 5));
      ...
      {data && (
        <div>
          ...
          <div>
            ...
            />
            (배송비는 5개당 {basicShipppingFees.toLocaleString()}원씩
            추가됩니다.)
            <br />
